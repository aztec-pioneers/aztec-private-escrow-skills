...
- `SinglePrivateMutable<StateNote, Context>` for `CREATED`, `OPEN`, `VOID`, and `FILLED`;
...
- `OrderFilled {}` delivered to `self.address` with `ONCHAIN_CONSTRAINED`;
- no custom order-level deposit/fill nullifiers.
...
Token method names in the default source are the selected token adapter surface. If the selected token standard differs, update both the Noir calls and TypeScript SDK helpers by capability: private deposit, private refund, private payout, and private transfer-to-commitment.
