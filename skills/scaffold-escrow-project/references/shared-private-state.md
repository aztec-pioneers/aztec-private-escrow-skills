...
Role secrets are the exception: a `RoleSecretNote` is caller-owned private state used to authenticate a maker/taker/filler pseudonym. Store the pseudonym in shared config or lifecycle state, not the role-secret note itself.
...
| Caller-owned role secret | `Owned<PrivateImmutable<RoleSecretNote, Context>, Context>` |
...
For private role checks, read the caller's `RoleSecretNote`, compute its pseudonym, and compare it to the role pseudonym stored in contract-owned config or lifecycle state.
