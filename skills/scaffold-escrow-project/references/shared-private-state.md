...
| Single role secret | `SinglePrivateImmutable<RoleSecretNote, Context>` delivered to the role holder |
...
For private role checks, read the caller's `RoleSecretNote`, assert its `owner` field equals the caller, compute its pseudonym, and compare it to the role pseudonym stored in contract-owned config or lifecycle state.
