...
These files target Aztec `4.2.0`, `EmbeddedWallet`, NodeNext runtime `.js` import suffixes, package imports such as `@aztec-otc-desk/contracts`, and `additionalScopes` for private cross-contract reads. `contract.ts` exports role-secret helpers including `retrieveRoleSecret`, which scans caller-addressed `RoleAdded` private events and returns the recovered secret. `manifest.ts` exports an `EscrowManifest` class for minimal secret-contract handoff, wallet registration, JSON, base64, and encrypted transport. When adapting the project name, update both `packages/contracts/package.json` and `packages/contracts/tsconfig.json` paths so tests can import through `@<project-scope>/contracts`.
